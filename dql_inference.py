...
class DQNAgent:
    def __init__(self, state_size, action_size):
        ...
        self.memory = deque(maxlen=10000)  # Not used in inference mode
        self.gamma = 0.95    # discount rate (not used in inference)
        self.epsilon = 0.05  # Small epsilon for minimal exploration during visualization
...
class BattlePainterInference:
    def __init__(self, server_uri="ws://localhost:9080/agent-client", model_path=None):
        ...
        # Episode tracking
        self.episode = 0
        self.max_episodes = 1 # Set to 1 for visualization, can be increased for multiple runs
        ...
        while self.episode < self.max_episodes:
            try:
                async with websockets.connect(self.server_uri) as websocket:
                    logger.info(f"Connected to game server. Running visualization episode {self.episode + 1}")
                    self.done = False
                    self.current_coverage = 0
                    self.previous_coverage = 0
                    ...
                    while not self.done:
                        ...
                        if game_data["event"] == "STATE_UPDATE":
                            ...
                            # Update best coverage for tracking
                            ...
                            logger.info(f"Best coverage so far: {self.best_coverage * 100:.2f}%")
